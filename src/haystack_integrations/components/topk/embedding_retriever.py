...
class TopKEmbeddingRetriever:
    """
    Retriever that performs dense vector similarity search against a TopKDocumentStore.
    """
    ...
    def run(
        ...
        :param query_embedding: Query vector produced by an embedder component.
        ...
        query = select("content", "blob", score=fn.vector_distance("embedding", f32_list(query_embedding)))
        ...
        query = query.topk(field("score"), effective_k, asc=False)

        results = self._document_store._collection().query(query)
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "TopKEmbeddingRetriever":
